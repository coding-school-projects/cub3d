...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2024/08/26 15:55:11 by pchennia          #+#    #+#             */
/*   Updated: 2024/11/07 10:56:50 by pchennia         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
// bpp-bits pper pixel
// sl-number of bytes in a single row of the image
// end-endianness
...
void	calculate_ray_vector(t_all *data)
{
	...
	half_window_width = data->win.x / 2;
	ray_angle = ((double)data->ray.angle_index - half_window_width) \
									* 30 / half_window_width;
	ray_angle = ray_angle * M_PI / 180;
	data->ray.x = data->dir.x * cos(ray_angle) - data->dir.y * sin(ray_angle);
	data->ray.y = data->dir.y * cos(ray_angle) + data->dir.x * sin(ray_angle);
}
